...
function addRasterToMap(item,viz,name,visible,label,fontColor,helpBox){
    ...
    var layerList = document.querySelector("layer-list");
...
/////////////////////////////////////////////////////
//Function to add ee object ot map
function addRESTToMap(tileURLFunction,name,visible,maxZoom,helpBox){
...
function addDynamicToMap(baseUrl1,baseUrl2, minZoom1,minZoom2,name,visible,helpBox){
...
function addToMap(item,viz,name,visible,label,fontColor,helpBox){
    if(canAddToMap){
    try{var t = item.bandNames();
        
        addRasterToMap(item,viz,name,visible,label,fontColor,helpBox);}
    catch(err){
        try{
        item = ee.Image().paint(item,3,3);
        addToMap(item,viz,name,visible,label,fontColor,helpBox)
      }
    catch(err){
...
function mp(){
  this.addLayer = function(item,viz,name,visible,label,fontColor,helpBox){
    addToMap(item,viz,name,visible,label,fontColor,helpBox);
  };
  this.addREST = function(tileURLFunction,name,visible,maxZoom,helpBox){
    addRESTToMap(tileURLFunction,name,visible,maxZoom,helpBox);
  };
    ...
    updateArea = function(){
      ...
      totalArea = totalArea*unitMultiplier;
        ...
        // if(totalArea > 0){
          ...
          $( "#distance-area-measurement" ).html(totalWithArea.toString()+' '+polyString+' <br>'+totalArea +' '+unitName );
